...
func ErrOrderNotFound() *AppError {
	return &AppError{Code: "order.not_found", Message: "order not found"}
}

func ErrEmptyOrder() *AppError {
	return &AppError{Code: "order.empty", Message: "order must contain at least one item"}
}
...
func ErrInvalidItemQuantity(itemIndex int) *AppError {
	return &AppError{
		Code:    "order.invalid_item_quantity",
		Message: fmt.Sprintf("item %d: quantity must be greater than 0", itemIndex),
	}
}
...
func ErrInvalidItemPrice(itemIndex int) *AppError {
	return &AppError{
		Code:    "order.invalid_item_price",
		Message: fmt.Sprintf("item %d: price must be greater than 0", itemIndex),
	}
}
