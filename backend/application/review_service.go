...
func (s *ReviewService) SubmitReview(ctx context.Context, userID string, productID int64, req dto.CreateReviewRequest) error {
	if req.Rating < 1 || req.Rating > 5 {
		return &domain.ValidationError{Message: "rating must be between 1 and 5"}
	}
	req.Comment = strings.TrimSpace(req.Comment)
	if len(req.Comment) < 10 {
		return &domain.ValidationError{Message: "comment must be at least 10 characters"}
	}
	if len(req.Comment) > 1000 {
		return &domain.ValidationError{Message: "comment must be at most 1000 characters"}
	}
	...
	if !purchased {
		return &domain.ValidationError{Message: "you must purchase this product before leaving a review"}
	}
	...
	if exists {
		return &domain.ConflictError{Message: "you have already reviewed this product"}
	}
