...
func (s *OrderService) CreateOrder(ctx context.Context, userID string, req dto.CreateOrderRequest) (port.Order, error) {
	if len(req.Items) == 0 {
		return port.Order{}, &domain.ValidationError{Message: "order must contain at least one item"}
	}
	...
	for i, item := range req.Items {
		if item.Quantity <= 0 {
			return port.Order{}, &domain.ValidationError{
				Message: fmt.Sprintf("item %d: quantity must be greater than 0", i+1),
			}
		}
		if item.PriceAtPurchase <= 0 {
			return port.Order{}, &domain.ValidationError{
				Message: fmt.Sprintf("item %d: price must be greater than 0", i+1),
			}
		}
...
func (s *OrderService) GetOrder(ctx context.Context, userID, orderID string) (port.Order, error) {
	...
	if err != nil {
		return port.Order{}, &domain.NotFoundError{Resource: "order"}
	}
	return order, nil
}
